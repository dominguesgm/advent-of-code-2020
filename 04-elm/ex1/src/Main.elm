...
headAndRest : List (List String) -> (List String, List (List String))
headAndRest list =
  case list of
    h::r -> (h, r)
    [] -> ([], [])

lineValidation : String -> List (List String) -> List (List String)
lineValidation item acc =
  let (head, rest) = (headAndRest acc)
  in
  if item == "" then
    [] :: acc
  else
    (append (split " " item) head) :: rest


passportsArray input = second (headAndRest
...
accumulateValid item acc = 
  if item then
    acc + 1
  else
    acc

countValidPassports input = foldl accumulateValid 0 (getValidPassports input)

-- MAIN


main =
  Browser.sandbox { init = result, update = update, view = view }
...
result : Model
result = let _ = Debug.log "value" (countValidPassports data)
      in
  0
