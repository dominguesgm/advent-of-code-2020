...
let readPieces = (): array(piece) => {
    ...
    let result = listOfLines^ |> Array.fold_left((acc: fileAcc, line: string) => {
        switch(line) {
        | "" =>
            {currentPiece: {id: 0, opposingEdges: [||]}, listOfPieces: Array.append(acc.listOfPieces, [|acc.currentPiece|])}
        | x when String.length(x) == 10 && String.compare(String.sub(x, 0, 4), "Tile") == 0 =>
            {...acc, currentPiece: {id: Stdlib.int_of_string(String.sub(x, 5, 4)), opposingEdges: [||]}}
        | a when String.length(a) == 10 && Array.length(acc.currentPiece.opposingEdges) == 0 =>
            {...acc, currentPiece: {...acc.currentPiece, opposingEdges: [| [|line|], [| String.make(1, line.[0]), String.make(1, line.[9]) |] |] }}
    ...
    }, {listOfPieces: [||], currentPiece: {id: 0, opposingEdges: [||]}});
